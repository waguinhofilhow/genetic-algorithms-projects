...
double schaffersf6(double* x, int n) {
    
    double numerator = pow(sin(sqrt(x[0] * x[0] + x[1] * x[1])), 2.0) - 0.5;
    double denominator = pow(1.0 + 0.001 * (x[0] * x[0] + x[1] * x[1]), 2.0);
...
double** startPopulation(int n, int populationSize) {
    ...
    for (int i = 0; i < populationSize; i++) {
        ...
        for (int j = 0; j < n; j++) {
            population[i][j] = (10 - (double) rand() / RAND_MAX * 20.0);
        }
    }
    return population;
}
...
double* findBestInGeneration(double** generation, int populationSize, int index, int n) {
    ...
    double f0 = schaffersf6(generation[0], n);
    double f1 = schaffersf6(generation[1], n);
    ...
    for (int i = 2; i < populationSize; i++) {
        double fit = schaffersf6(generation[i], n);
        if (fit > schaffersf6(best, n)) {
            secondBest = best;
            best = generation[i];
        } else if (fit > schaffersf6(secondBest, n)) {
...
void maxschaffersf6(int n, int populationSize, int maxGenerations) {
    ...
    for (int i = 0; i < populationSize; i++) {
        generationFitness[i] = normalizedFitness(schaffersf6(generation[i], n));
    }
    ...
    while (generationCount < maxGenerations) {
        ...
        for (int i = 2; i < populationSize; i += 2) {
            ...
            nextGen[i] = c1;
            nextFitness[i] = normalizedFitness(schaffersf6(c1, n));

            if (i + 1 < populationSize) {
                nextGen[i + 1] = c2;
                nextFitness[i + 1] = normalizedFitness(schaffersf6(c2, n));
            } else {
                free(c2);
            }
        }
    ...
    printf("- Valor: %f - Fitness: %f\n", schaffersf6(best, n), normalizedFitness(schaffersf6(best, n)));
    free(best);
...
int main() {
    ...
    maxschaffersf6(n, populationSize, maxGenerations);
    return 0;
}